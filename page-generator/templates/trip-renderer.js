    ...
    setHTML('modal-attractions', attractions.length
      ? attractions.map(function (a) {
          return '<article class="rounded-2xl border border-stone-200 bg-white p-4 shadow-soft">' +
            ...
            (a.bookingUrl ? '<div class="mt-3"><a href="' + esc(a.bookingUrl) + '" target="_blank" class="inline-block rounded bg-green-500 px-2 py-1 text-sm text-white">查看详情</a></div>' : '') +
  ...
  function render(data) {
    ...
    setHTML('hotel-grid', data.hotels.map(function (h) {
      return '<article class="overflow-hidden ' + (theme().css && theme().css.card || '') + '">' +
        ...
        (h.bookingUrl ? '<div class="mt-4"><a href="' + esc(h.bookingUrl) + '" target="_blank" class="inline-block rounded bg-blue-500 px-3 py-1 text-white">立即预订</a></div>' : '') +
    ...
    setHTML('attraction-grid', data.attractions.map(function (a) {
      return '<article class="overflow-hidden ' + (theme().css && theme().css.card || '') + '">' +
        ...
        (a.bookingUrl ? '<div class="mt-4"><a href="' + esc(a.bookingUrl) + '" target="_blank" class="inline-block rounded bg-green-500 px-2 py-1 text-sm text-white">查看详情</a></div>' : '') +
